@extends('layout')
@section('title', '又拍云-文件删除')
@section('content')
    <h1 id="overview" class="page-header">文件删除</h1>
    @if(count($filelist)>0)
        <div class="row upfilelist">
            @foreach ($filelist as $list)
                ...
                @else
                    <div class="col-xs-6 col-md-4 img-thumbnail" style="position: relative">
                        <a href="{!! $pathstr !!}{!! $path !!}{!!$list['name']!!}" target="_blank">
                            <img src="{!! $pathstr !!}{!! $path !!}{!!$list['name']!!}"/>
                        </a>
                        <a class="btn btn-danger" style="position:absolute; right: 10px; top:10px;" href="/upfiledelAction?delfile={!! $path !!}{!!$list['name']!!}">X</a>
                    </div>
                @endif
            @endforeach
        </div>
